...
while k<iteration
    ...
    x2=[im01_loc(2,randompick(1)) im01_loc(2,randompick(2)) im01_loc(2,randompick(3)) im01_loc(2,randompick(4))];
    y2=[im01_loc(1,randompick(1)) im01_loc(1,randompick(2)) im01_loc(1,randompick(3)) im01_loc(1,randompick(4))];
    x1=[im02_loc(2,randompick(1)) im02_loc(2,randompick(2)) im02_loc(2,randompick(3)) im02_loc(2,randompick(4))];
    y1=[im02_loc(1,randompick(1)) im02_loc(1,randompick(2)) im02_loc(1,randompick(3)) im02_loc(1,randompick(4))];
    A_matrix=zeros(8);
    ...
    for i=1:allmatched_size
        x=im02_loc(2,i);y=im02_loc(1,i);x2=im01_loc(2,i);y2=im01_loc(1,i);
    ...
    if I_now>I_best
        I_best=I_now;homography_matrix_best=homography_matrix;k=1;
    end
    k=k+1;times=times+1;
end
H_T=projective2d(homography_matrix_best');
result=imwarp(img2,H_T);
% figure
% imshow(result);
...
y_rightup=(homography_matrix_best(2,1)*im01_size(2)+homography_matrix_best(2,2)+homography_matrix_best(2,3))/(homography_matrix_best(3,1)*im01_size(2)+homography_matrix_best(3,2)+homography_matrix_best(3,3));
x_leftdown=(homography_matrix_best(1,1)+homography_matrix_best(1,2)*im01_size(1)+homography_matrix_best(1,3))/(homography_matrix_best(3,1)+homography_matrix_best(3,2)*im01_size(1)+homography_matrix_best(3,3));
x_offset=min(x_leftdown,x_leftup);y_offset=min(y_rightup,y_leftup);
canvas=im2uint8(zeros(im01_size(1)+result_size(1),2*im01_size(2)+2*result_size(2),3));
...
for y=1:im01_size(1)
    for x=1:im01_size(2)
        ...
        if canvas(y_real,x_real,:)>0
            canvas(y_real,x_real,:)=canvas(y_real,x_real,:)/2+img1(y,x,:)/2;
        else
            canvas(y_real,x_real,:)=img1(y,x,:);
        end
    end
end
