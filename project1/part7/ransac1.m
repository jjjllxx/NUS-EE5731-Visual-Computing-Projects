...
function [ final_canvas ] = ransac1( loc1, loc2, img01, img02, allmatched)
...
% drawinlierMatch(bestmatch, img1, img2)
H_T=projective2d(homography_matrix_best');
result=imwarp(img01,H_T);
% figure(1)
% imshow(result);

%caculate the offset on the canvas
im02_size=size(img02);im01_size=size(img01);result_size=size(result);
...
y_rightup=(homography_matrix_best(2,1)*im01_size(2)+homography_matrix_best(2,2)+homography_matrix_best(2,3))/(homography_matrix_best(3,1)*im02_size(2)+homography_matrix_best(3,2)+homography_matrix_best(3,3));
x_leftdown=(homography_matrix_best(1,1)+homography_matrix_best(1,2)*im01_size(1)+homography_matrix_best(1,3))/(homography_matrix_best(3,1)+homography_matrix_best(3,2)*im02_size(1)+homography_matrix_best(3,3));
x_offset=min(x_leftdown,x_leftup);y_offset=min(y_rightup,y_leftup);
canvas=im2uint8(zeros(2*im01_size(1)+2*result_size(1),4*im01_size(2)+4*result_size(2),3));
...
for y=1:im02_size(1)
    for x=1:im02_size(2)
        ...
        if canvas(y_real,x_real,:)>0 & img02(y,x,:)>0
            canvas(y_real,x_real,:)=canvas(y_real,x_real,:)/2+img02(y,x,:)/2;        
        else
            canvas(y_real,x_real,:)=max(img02(y,x,:),canvas(y_real,x_real,:));
        end
    end
end
