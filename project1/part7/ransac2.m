...
function [ final_canvas ] = ransac2( loc1, loc2, img01, img02, allmatched)
...
result=imwarp(img02,H_T);
% figure
% imshow(result);
% caculate the offset on the canvas
im01_size=size(img01);im02_size=size(img02);result_size=size(result);
...
canvas=im2uint8(zeros(4*im02_size(1)+4*result_size(1),4*im02_size(2)+4*result_size(2),3));
...
for y=1:im01_size(1)
    for x=1:im01_size(2)
        ...
        if canvas(y_real,x_real,:)>0 & img01(y,x,:)>0
            canvas(y_real,x_real,:)=canvas(y_real,x_real,:)/2+img01(y,x,:)/2;        
        else
            canvas(y_real,x_real,:)=max(img01(y,x,:),canvas(y_real,x_real,:));
        end
    end
end
